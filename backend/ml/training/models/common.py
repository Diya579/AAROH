...
class ModelExportManager:
    ...
    def save_model(
        ...
        # 5. Tokenizer metadata or config
        ...
        if weights_data is not None:
            # If PyTorch is available and weights_data is a state_dict or model
            try:
                import torch
                ...
                # Fallback serialized representation if torch is not installed
                with open(out_path / "weights_meta.json", "w", encoding="utf-8") as f:
                    json.dump({"weights_saved": True, "type": str(type(weights_data))}, f, indent=2)
        else:
            # Create placeholder checkpoint indicator if dry-run
            with open(out_path / "weights_meta.json", "w", encoding="utf-8") as f:
                json.dump({"weights_saved": False, "status": "initialized"}, f, indent=2)
    ...
    def load_model_metadata(model_dir: Path | str) -> dict[str, Any]:
        ...
        for fname in ["metadata.json", "config.json", "label_mapping.json", "metrics.json"]:
            fpath = m_dir / fname
            if fpath.exists():
...
class CheckpointManager:
    ...
    def save_checkpoint(
        ...
        try:
            ...
            save_dict = {
                **ckpt_payload,
                "model_state_dict": model_state if isinstance(model_state, dict) else (
                    model_state.state_dict() if hasattr(model_state, "state_dict") else model_state
                ),
            }
            if optimizer_state is not None:
                save_dict["optimizer_state_dict"] = (
                    optimizer_state.state_dict() if hasattr(optimizer_state, "state_dict") else optimizer_state
                )
            torch.save(save_dict, ckpt_path)
            if is_best:
                torch.save(save_dict, self.checkpoint_dir / "best_checkpoint.pt")
        except ImportError:
    ...
    if labels and len(labels) == len(embeddings):
        ...
        if len(groups) >= 2:
            ...
            if norm1 > 0 and norm2 > 0:
                sim = dot / (norm1 * norm2)
                # Separation is 1 - cosine similarity
